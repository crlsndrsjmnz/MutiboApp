 ...
 *        concurrently.
 * 
 *        SessionIntentService receives an Intent containing a Session
...
public class SessionIntentService extends IntentService {
	 ...
	 * service runs in.
	 */
	public SessionIntentService(String name) {
		super(name);
	}
	...
	public static Intent makeIntent(Context context, Handler handler, Session currSession,
		...
		return DownloadUtils.makeMessengerIntent(context,
				SessionIntentService.class, handler, currSession, session);
	}
	...
	protected void onHandleIntent(Intent intent) {
		...
		DownloadUtils.saveAndRespond(
				new Session(), 
				CloudManager.SESSION_SERVICE,
				(Messenger) intent.getExtras().get(CloudManager.MESSENGER_KEY),
				(Session) intent.getExtras().get(CloudManager.NEWSESSION_KEY));
